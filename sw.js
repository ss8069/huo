...
self.addEventListener('fetch',e=>{
  ...
  if(isStatic){
    ...
    e.respondWith(caches.match(e.request).then(r=>r||fetch(e.request).then(res=>{if(res.ok||res.type==='opaque'){const cp=res.clone();e.waitUntil(caches.open(C).then(c=>c.put(e.request,cp)))}return res})));
    return;
  }
  // network-first for app files, fallback to cache
  e.respondWith(fetch(e.request).then(res=>{if(res.ok){const cp=res.clone();e.waitUntil(caches.open(C).then(c=>c.put(e.request,cp)))}return res}).catch(()=>caches.match(e.request).then(r=>r||caches.match('./index.html'))));
});
